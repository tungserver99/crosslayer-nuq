...
def accumulate_saliency_weighted_hessians(
    ...
    output_folder: str,
    num_groups: int,
    initial_activations_cache_path: str = None
) -> bool:
    """
    ...
    if initial_activations_cache_path is not None:
        logging.info(f"Saving initial layer inputs for EndLoss reuse to {initial_activations_cache_path}")
        os.makedirs(os.path.dirname(initial_activations_cache_path), exist_ok=True)
        torch.save({"inps": [inp.cpu() for inp in inps], "forward_args": forward_args}, initial_activations_cache_path)
    ...
    for l in range(num_layers):
        ...
        logging.info(f"[Layer {l}] Accumulating GuideQuant weighted XTX/Hessian")
        ...
        if len(devices) == 1:
            ...
            logging.info(f"[Layer {l}] Running saliency-weighted XTX forward pass")
