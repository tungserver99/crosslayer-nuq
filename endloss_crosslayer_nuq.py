import argparse
from any_precision.quantization import endloss_crosslayer_nuq
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run EndLoss sequential cross-layer LNQ + GuidedQuant")
    ...
    parser.add_argument("--overwrite_hessians", action="store_true",
                        help="Whether to overwrite the GuidedQuant Hessian cache")
    ...
    parser.add_argument("--signed_gradients_path", type=str, default=None,
                        help="Optional directory containing signed output gradients")
    ...
    endloss_crosslayer_nuq(**{k: v for k, v in args.__dict__.items() if v is not None})
